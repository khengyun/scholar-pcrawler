...
class ScholarScraper:
    ...
    def _get_abstract(self, paper_url):
        response = self._retry_request(self.base_url + paper_url)
        if response:
            soup = BeautifulSoup(response.text, 'html.parser')
            abstract_element = soup.find('div', {'class': 'gsh_csp'})
            abstract = abstract_element.text.strip() if abstract_element else 'N/A'
            return abstract
        else:
            print(f'Error fetching abstract for {paper_url}')
            return 'N/A'
    ...
    def get_scholar_papers(self, user_id, limit=None):
        ...
        if response:
            ...
            for i, paper_element in enumerate(paper_elements):
                ...
                # Fetch abstract
                abstract = self._get_abstract(paper_url)
